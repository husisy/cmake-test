...
**问题描述**：在step5中我希望cmake能够导入step4生成的GPU静态库，然后将`main.cpp`链接至该静态库并完成编译，但执行`cmake --build .`命令时抛错
...
前三个step用于说明：在没有GPU代码时，个人对于cmake的运行机制应该不存在明显的偏差

仿照前三个step，只是将其中的打印函数替换为GPU代码；即使用nvcc、cmake两个工具来完成相同的步骤，但`step5`失败
